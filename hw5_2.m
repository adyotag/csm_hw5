...
q_hist =  zeros(length(times)+1,3,3);       % back stress history
alpha_hist =  zeros(length(times)+1,1);     % cumulative plastic strain history
stress_hist = zeros(length(times)+1,3,3);   % stress history
strain_hist = zeros(length(times)+1,3,3);   % stores elastic strain history

% Perform return mapping algorithm
for i = 1:length(times)
    strain_hist(i+1,:,:) = nextStep(i, squeeze(eps_p_hist(i,:,:)) );  % obtain strain at next step
    ...
    if next_yF_tr < 0       % Elastic case
        ...
        next_n = next_ksi_tr/mag(next_ksi_tr);  % Updates normal tensor
        delta_gamma = next_yF_tr/(K+H+2*mu);    % Computes the plastic strain multiplier
...
% Plots
figure;
plot(strain_hist(:,1,1)+eps_p_hist(:,1,1),stress_hist(:,1,1)); hold on
plot(strain_hist(:,2,2)+eps_p_hist(:,2,2),stress_hist(:,2,2));
plot(strain_hist(:,3,3)+eps_p_hist(:,3,3),stress_hist(:,3,3));
...
figure;
plot(strain_hist(:,2,2)+eps_p_hist(:,2,2),strain_hist(:,1,1)+eps_p_hist(:,1,1));
title('Lateral Plastic Strain against Axial Strain');
ylabel('Lateral Strain, $$\epsilon_{11}$$', 'interpreter', 'latex');
xlabel('Axial Strain, $$\epsilon_{22}$$', 'interpreter', 'latex');

figure;
plot(strain_hist(:,2,2)+eps_p_hist(:,2,2), eps_p_hist(:,2,2));
...
figure;
plot(strain_hist(:,2,2)+eps_p_hist(:,2,2), alpha_hist);
...
function r = nextStep(i, eps_p) % Strain
